...
copy enum Toml {
    ...
    Number(string);
    String(string);
    Array(string); // inline TOML array text
    Table(string); // full TOML document/table text
}
...
fn parse_scalar_token(t: string) -> Toml {
    ...
    while i < n {
        ...
        if !(is_digit(b) || b == byte(".")) { ok_num = false; break; }
        i = i + one_i64();
    }
    ...
    *idx = *idx + one_i64();
    ...
    loop {
        ...
        match r {
            ...
            Result.Err(e) => { has_err = true; err_val = e; },
            _ => { has_err = true; },
        }
