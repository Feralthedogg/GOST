...
fn blt_digit_char_i64(d: i64) -> string {
    if d == 0 { return "0"
 }
    if d == 1 { return "1"
 }
...
fn blt_i64_to_string(v: i64) -> string {
    if v == 0 { return "0"
 }
    if v < 0 { return "0"
 }
    ...
    while n > 0 {
        let d: i64 = n % 10
        out = string_concat(blt_digit_char_i64(d), out)
        n = n / 10
    }
    return out
}
...
fn blt_accept_loop(listener: ref[TcpListener], accepted: mutref[own[i32]], stop: ref[own[i32]], done: ref[chan[i32]]) {
    loop {
        let s0 = own_borrow[i32](&*stop)
        if *s0 != 0 {
            send(*done, 1)
            return
        }
        match tcp_accept(*listener) {
            Result.Ok(conn) => {
                let s1 = own_borrow[i32](&*stop)
                if *s1 != 0 {
                    let _ = tcp_close(conn)
                    send(*done, 1)
                    return
                } else {
                    let c = own_borrow_mut[i32](&mut *accepted)
                    *c = *c + 1
...
fn blt_client_once(addr: string) -> i32 {
    ...
    let _ = tcp_close(conn)
    if string_len(resp) == blt_zero_i64() {
        return 3
    }
    if blt_starts_with(resp, "HTTP/1.1 200") {
        return 0
    }
    return 4
}
...
fn main() -> i32 {
    let start_ms = __gost_now_ms()
    let addr = "127.0.0.1:18080"
    let workers: i32 = 1
    let iters_per_worker: i32 = 10000
    ...
    let accepted_counter = own_new[i32](0)
    let accept_stop = own_new[i32](0)
    let accept_done = make_chan[i32](1)
    go blt_accept_loop(&listener, &mut accepted_counter, &accept_stop, &accept_done)
    ...
    let stop = own_borrow_mut[i32](&mut accept_stop)
    *stop = 1
    let _ = tcp_close_listener(listener)
    match tcp_connect(addr) {
        Result.Ok(conn) => {
            let _ = tcp_close(conn)
        },
    ...
    select {
        case recv(accept_done) => |_, _| {},
    }

    let accepted: i32 = *own_borrow[i32](&accepted_counter)
    ...
    println("=====================================")
    let elapsed_ms = __gost_now_ms() - start_ms
    println(string_concat("elapsed_ms: ", blt_i64_to_string(elapsed_ms)))
    println("load test finished")
    let _ = __gost_process_exit(0)
    return 0
}
